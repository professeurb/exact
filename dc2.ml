...
let ec = EC.create ()
let _ = EC.add_primary ec 1
let _ = EC.add_primary ec 2
let _ = EC.add_secondary ec 3
let _ = EC.add_shape ec 1 [ 1; 2; 3 ]
let _ = EC.add_shape ec 2 [ 1 ]
let _ = EC.add_shape ec 3 [ 2 ]
let pb = make ec

let _ =
  (* [(2, [1; 3], 6); (1, [1; 2], 6)] *)
  EC.get_primary_cols ec |> List.of_seq
  |> List.map (fun col -> (col, EC.get_rows_of_col ec col, size_of_col ec col))

let _ =
  (* [(2, [1], [1; 2], 6); (3, [2], [1; 3], 6); (1, [1; 2], [1; 2; 3], 10)] *)
  EC.get_rows ec |> List.of_seq
  |> List.map (fun row ->
         ( row,
           EC.get_primary_cols_of_row ec row,
           EC.get_rows_of_row ec row,
           size_of_row ec row ))
